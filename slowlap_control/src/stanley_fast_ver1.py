...
def stanley_control(x, y, yaw, v, map_xs, map_ys, map_yaws, L):
	...
	idx_waypoints = len(map_xs)
	#print(idx_waypoints)
	for i in range(idx_waypoints):
		waypoints.append([map_xs[i],map_ys[i]])
		curve.append(waypoints)
	curve_tmp = np.array(curve)
	print(curve_tmp)
	#plt.plot(curve_tmp[:,0], curve_tmp[:,1])
		#rst=np.gradient(curve_tmp)
		#curve_gradient = np.mean(rst[i])
		#print("curve_gradient_x:",curve_gradient)
...
def callback2(msg):
	...
	# print(yaw)
	steer = stanley_control(msg.pose.pose.position.x, msg.pose.pose.position.y, yaw, 2, map_x, map_y, map_yaw, 0.8)
...
if __name__ == '__main__':
    rospy.init_node('stanley_control_ver1')
    rospy.Subscriber('/loop_path',Path,callback)
    rospy.Subscriber('/odom',Odometry,callback2)
    steer_pub = rospy.Publisher('/steer_ver1',Float32,queue_size=1)
